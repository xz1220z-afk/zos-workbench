...
- **决策计数一致性**：工作首页主标题、侧栏徽标与 KPI 统一只统计当前未处理且需要本人判断的决策；已处理历史保留在历史区，不进入今日数量。
- **AI 故障可诊断**：文字问答与实时语音把 OpenAI 故障限制为安全类别，并显示可操作提示；不向前端暴露原始上游响应或密钥。
- **语音能力保真**：AI 请求失败不再清除 Chrome 的 WebRTC/麦克风能力；服务故障显示可重试状态，不伪装成浏览器不支持。
- **数据保护**：不修改任务、日历、Agent、知识、业务数据或决策历史；发布前回滚点为 `zos-workbench-v2.12.0-pre-production-fixes`。
- **版本保护**：PWA 缓存、清单、入口和浏览器模块统一为 `2.12.1`。

## v2.12.0（发布候选）

- **AI Office 实时席位**：基于现有 Agent 身份卡、运行和任务记录动态展示组织、部门、职责与当前状态。
- **统一执行台账**：安全汇总 AI 活动、Agent 运行、任务归档与待确认动作，不保存正文、语音、知识正文或密钥。
- **AI 推进提醒**：在工作首页提示逾期、目标缺口和未落地的下一步，并只生成可编辑任务草案。
- **权限边界**：文字/语音/知识入口显示真实能力状态；外部动作继续要求预览与明确确认。
- **隐私隔离**：REL-001 只在私密关系入口展示，公共视图和台账脱敏。
- **版本保护**：发布前标签 `zos-workbench-v2.11.0-pre-ai-office`；PWA 缓存、清单、入口和浏览器模块统一为 `2.12.0`。
