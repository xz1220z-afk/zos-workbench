...
### 老板登录、实时同步与 ChatGPT 双语音

- **入口与范围**：四端打开工作台先进入独立登录页；授权后恢复原路由和全部既有模块。登录页不是第二套工作台，也不改变原导航、业务数据结构或用户记录。
- **登录边界**：服务端以 `ZOS_OWNER_USER_ID` 精确校验老板身份；匿名请求为 401、已登录非老板为 403。工作台只可记住邮箱和 Supabase 会话，绝不保存明文密码；OTP 禁止自动创建新用户。
- **实时同步**：`zos_records` 仍按用户 RLS 隔离。Realtime 仅通知变化，随后复用已有拉取、合并、冲突、墓碑和本地兼容投影，不以事件载荷覆盖权威数据。
- **双语音**：快捷语音把本人主动提交的转写交给既有 AI；实时语音通过 WebRTC 和服务端创建的 OpenAI Realtime 会话交互，支持字幕、静音、打断、结束和有限重连。
- **知识与执行边界**：仅发送当前允许的任务上下文和经批准知识摘要；不发送整库、原始业务 JSON 或原始音频。消息、飞书、发布、外部日历、删除、付款等仍只生成预览并等待明确确认。
- **实现模块**：`src/app/auth-gate.mjs`、`owner-session-client.mjs`、`authenticated-bootstrap.mjs`、`realtime-sync-signal.mjs`、`supabase-realtime-channel.mjs`、`voice-turn.mjs`、`realtime-voice.mjs`、登录视图、相关 Edge Functions 与迁移。
- **PWA 版本**：缓存名、入口、manifest、CSS 和浏览器模块统一为 `2.11.0`；认证、实时同步和两种语音所需静态模块进入原子缓存，不缓存会话、API 响应、SDP、音频或密钥。
- **回滚**：代码恢复点为 `zos-workbench-v2.10.0-pre-owner-auth`。语音异常时可单独停用新会话函数；实时订阅异常时退回定时/联网/前台刷新；任何回滚都不删除本机或云端记录。
