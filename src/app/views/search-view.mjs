...
export function render(container, viewModel = {}) {
  ...
  container.innerHTML = `<form class="global-search" id="globalSearchForm"><label for="globalSearchInput">搜索工作台</label><div><input id="globalSearchInput" name="query" value="${escapeHtml(viewModel.searchQuery || '')}" placeholder="搜索项目、商家、知识、情报和行动…" autocomplete="off"><button class="v13-action v13-action-primary">搜索</button></div><p>结果会明确标注事实源、知识元数据或个人私有内容。</p></form>
