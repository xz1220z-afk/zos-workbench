import { escapeHtml, renderState, VIEW_STATES } from './view-utils.mjs';
  ...
  container.innerHTML = `
    <div class="v13-grid">
      ${decisions.length ? decisions.map((item) => `<article class="v13-panel">
        <span class="v13-chip v13-severity-${escapeHtml(item.severity || 'medium')}">${escapeHtml(item.severity || '待判断')}</span>
        <h3>${escapeHtml(item.factSummary || item.title)}</h3>
        <p>AI 建议：${escapeHtml(item.recommendedAction || '暂无建议')}</p>
    ...
    ${preview ? `<aside class="v13-panel v13-approval-preview" role="dialog" aria-modal="true" aria-label="飞书更新预览">
      <h3>飞书更新预览</h3><p>${escapeHtml(preview.fieldName)}</p>
      <div class="v13-before-after"><span>${escapeHtml(preview.before)}</span><span class="v13-before-after-arrow">→</span><strong>${escapeHtml(preview.after)}</strong></div>
