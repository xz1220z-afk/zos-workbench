...
function card(item) {
  ...
  return `<article class="intelligence-card" data-intelligence-id="${escapeHtml(item.externalId)}">
    ...
    <div class="intelligence-actions"><button class="v13-action" data-intelligence-status="read" data-intelligence-id="${escapeHtml(item.externalId)}">标记已读</button><button class="v13-action v13-action-primary" data-intelligence-status="actioned" data-intelligence-id="${escapeHtml(item.externalId)}">转为行动</button></div>
  </article>`;
}
...
export function render(container, viewModel = {}) {
  ...
  container.innerHTML = `<div class="intelligence-toolbar">
    ...
    <div class="intelligence-source-note">来源：飞书 ZOS 情报候选池 / Supabase 私有缓存 · 只保存摘要与判断，不保存文章正文</div>
