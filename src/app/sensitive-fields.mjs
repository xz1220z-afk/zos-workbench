const SENSITIVE_FIELD = /(password|passcode|access[_-]?token|refresh[_-]?token|authorization|api[_-]?key|anon[_-]?key|secret|service[_-]?role|private[_-]?key|credential|cookie|session(?:[_-]?(?:id|token|key))?$)/i;
