import { fetchBusinessData, fetchWanjiaSchema } from '../business-data-client.mjs?v=2.12.1';
import { createSupabaseAuth } from '../supabase-auth.mjs?v=2.12.1';
import { createSupabaseTransport } from '../supabase-transport.mjs?v=2.12.1';
import { createFeishuApprovalClient } from './feishu-approvals.mjs?v=2.12.1';
import { createOperatingLoop } from './operating-loop.mjs?v=2.12.1';
import { createSyncController } from './sync-controller.mjs?v=2.12.1';
import { createPushClient } from './push-notifications.mjs?v=2.12.1';
import { createAiAssistantClient } from './ai-assistant-client.mjs?v=2.12.1';
import { createRealtimeSyncSignal } from './realtime-sync-signal.mjs?v=2.12.1';
import { createSupabaseRealtimeChannelFactory } from './supabase-realtime-channel.mjs?v=2.12.1';
import { createRealtimeSessionExchange } from './realtime-voice.mjs?v=2.12.1';
import { buildLocalSyncInput, LOCAL_ONLY_ENTITY_TYPES } from '../sync-engine.mjs?v=2.12.1';
...
export async function createBrowserOperatingRuntime({
  ...
  const operatingLoop = createOperatingLoop({
    ...
    refreshBusiness: (source) => fetchBusinessData({
      ...config, accessToken: session.accessToken, source, history: source === 'wanjia', fetchImpl,
    }),
