...
function consumedLessons(records) {
  ...
  let total = 0;
  for (const record of records || []) {
    const consumed = number(pick(record, ['已消课时', '课消课时', '完成课时', '核销课时']));
    if (consumed !== null) {
      proven = true;
      total += consumed;
      continue;
...
export function summarizeLingli(input = {}, options = {}) {
  ...
  return {
    ...
    students: countByStatus(input.students, ['学员状态', '在读状态', '状态'], ['在读', '就读中', '已报名', '正常', '学习中']),
    received: receivedThisMonth(input.finance, month),
