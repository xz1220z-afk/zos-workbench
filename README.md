...
每次新增功能和正式版本必须遵循 [版本备份与回读规则](docs/release-governance.md)，并追加 [功能账本](docs/feature-ledger.md)。代码以正式验收后的 Git 标签备份；用户记录继续使用应用内完整备份、本机检查点与安全合并恢复，两者不可混为一谈。
...
- 一键导出完整安全备份，统一收录新旧页面的 20 类本地业务状态、删除标记、同步基线与操作日志，不包含密码、会话或服务密钥
- 版本升级前自动保留最近 3 个本机检查点；导入前强制创建检查点并回读确认，存储失败时不会继续导入
- JSON 备份先校验结构、记录编号、重复项、大小与完整性，再安全合并；不会因恢复旧备份而删除当前设备后来新增的记录
- 可撤销上次恢复；撤销只恢复导入前的内容版本，并保留恢复后继续新建的内容
...
3. 重要改动前仍建议下载一份完整备份；导入采用安全合并，不会自动删除当前设备已有记录。
