...
function backup(label) {
  return { backupVersion: '2.0.2', state: { collections: { tasks: [{ id: label, title: label }] } } };
}
...
test('snapshot repository keeps three upgrade checkpoints and latest pre-import checkpoint', async () => {
  ...
  await repository.save({ kind: 'upgrade', appVersion: '2.0.2', backup: backup('u4') });
  await repository.save({ kind: 'pre-import', appVersion: '2.0.2', backup: backup('before-1') });
  const latest = await repository.save({ kind: 'pre-import', appVersion: '2.0.2', backup: backup('before-2') });
