...
assert.match(serviceWorker, /const CACHE_NAME = 'zos-workbench-v1\.8\.1';/,
...
assert.match(shellHtml, /src\/app\.mjs\?v=1\.8\.1/,
...
assert.match(indexHtml, /const APP_VERSION = '1\.8\.1'/,
  'The inline application version must match the current release');
assert.match(indexHtml, /工作台版本<\/div>[\s\S]{0,120}v1\.8\.1/,
