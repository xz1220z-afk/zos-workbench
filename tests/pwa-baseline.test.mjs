...
assert.match(serviceWorker, /const CACHE_NAME = 'zos-workbench-v2\.8\.2';/,
...
assert.match(shellHtml, /src\/app\.mjs\?v=2\.8\.2/,
  'The application bootstrap must bypass a stale controlling Service Worker cache');
assert.match(shellHtml, /assets\/app\.css\?v=2\.8\.2/,
...
assert.match(appSource, /\.\/app\/state-store\.mjs\?v=2\.8\.2/,
  'The startup-critical state module must bypass a stale controlling Service Worker cache');
assert.match(legacySource, /\.\/app\/router\.mjs\?v=2\.8\.2/,
...
assert.match(indexHtml, /const APP_VERSION = '2\.8\.2'/,
  'The inline application version must match the current release');
assert.match(indexHtml, /工作台版本<\/div>[\s\S]{0,120}v2\.8\.2/,
