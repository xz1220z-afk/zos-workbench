...
test('service worker caches the complete transitive browser module graph', async () => {
  ...
  assert.match(serviceWorker, /asset\.endsWith\('\.mjs'\) \? `\$\{asset\}\?v=2\.10\.0`/);
});
...
test('the shell captures raw pre-upgrade state before either application module can migrate it', async () => {
  ...
  assert.ok(capture >= 0);
  assert.ok(capture < html.indexOf('src/legacy-app.mjs?v=2.10.0'));
  assert.ok(capture < html.indexOf('src/app.mjs?v=2.10.0'));
});
