...
test('v2.8.1 release cache includes the Wanjia operations control tower module graph', async () => {
  ...
  assert.match(sw, /zos-workbench-v2\.8\.1/);
  assert.equal(manifest.version, '2.8.1');
  assert.match(app, /APP_VERSION\s*=\s*'2\.8\.1'/);
  assert.match(html, /\?v=2\.8\.1/);
  for (const asset of [
