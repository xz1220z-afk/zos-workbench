...
test('v2.7.3 release cache includes the complete Agent task and life module graph', async () => {
  ...
  assert.match(sw, /zos-workbench-v2\.7\.3/);
  assert.equal(manifest.version, '2.7.3');
  assert.match(app, /APP_VERSION\s*=\s*'2\.7\.3'/);
  assert.match(html, /\?v=2\.7\.3/);
  for (const asset of [
